...
pub mod bangumi_info {
    ...
    impl BangumiTitle {
        fn new(cn: Option<String>, en: Option<String>, jp: Option<String>) -> Self {
            BangumiTitle { cn, en, jp }
        }

        fn get_title(&self, lang: LANG) -> &str {
    ...
    impl BangumiInfo {
        ...
        fn parse(raw_title: &str) -> Option<BangumiInfo> {
            // let part_re = Regex::new(r"(?P<season>.*|\[.*])(?P<episode> -? \d+|\[\d+]|\[\d+.?[vV]\d]|第\d+[话話集]|\[第?\d+[话話集]]|\[\d+.?END]|[Ee][Pp]?\d+)(?P<others>.*)").unwrap();
        ...
        fn pop_season(raw_bangumi_name: &mut String) -> u32 {
            ...
            let season_re = Regex::new(
                r"(\d{1,2}(?:st|nd|rd|th) Season)|(Season \d{1,2})|(S\d{1,2})|[第].[季期]",
            )
            .unwrap();
        ...
        fn get_top_voted_item(map: &HashMap<String, i32>) -> Option<String> {
    ...
    mod test {
        ...
        fn get_titles() -> Vec<(&'static str, &'static str, u32, u32, &'static str)> {
            vec![
                // (
                //     "[ANi] 卡片战斗!! 先导者 will+Dress 第三季 - 09 [1080P][Baha][WEB-DL][AAC AVC][CHT][MP4]", 
                //     "卡片战斗!! 先导者 will+Dress", 3, 9, "ANi"
                // ), (
                //     "[GJ.Y] 卡片战斗!! 先导者 will+Dress 第三季 / Cardfight!! Vanguard: will+Dress Season 3 - 35 (Sentai 1920x1080 AVC AAC MKV)", 
                ...
                //     "神剑闯江湖 ―明治剑客浪漫谭―", 1, 10, "Lilith-Raws",
                // ), (
...
async fn main() -> Result<(), Box<dyn Error>> {
    // let url = "https://mikanani.me/RSS/Bangumi?bangumiId=3060";
    let url = "https://mikanani.me/RSS/Bangumi?bangumiId=3099";
    // let url = "https://mikanani.me/RSS/Bangumi?bangumiId=3070";
    let res = reqwest::get(url).await?;
    let status = res.status();
    if status.is_success() {
        let channel = Channel::read_from(&(res.bytes().await?)[..])?;
        let mut raw_titles = Vec::new();
        let link = channel.link.to_owned();
        for item in channel.items() {
            if let Some(raw_title) = item.title() {
                raw_titles.push(raw_title);
            }
        }

        BangumiInfo::new(raw_titles, link);
    }
    Ok(())
}
