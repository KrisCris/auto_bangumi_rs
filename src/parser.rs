...
impl Parser {
    ...
    pub fn to_bangumi(self, season: Option<u32>, offset: Option<u32>) -> Option<Bangumi> {
        match self.can_parse() {
            true => {
                ...
                let episode = self.episode().unwrap_or(0) + offset;
                Some(Bangumi::new(
                    title,
                    season,
                    episode,
...
mod test {
    ...
    fn test_name() {
        ...
        let b = p.unwrap().to_bangumi(Some(0), Some(5));
        println!("{}", b.unwrap());
    }
